...
def main(filename, temp):
    if os.path.isdir(filename):
        return os.system(f'{GHIDRA_PATH}ghidra')
    if '.gpr' in filename:
        os.system(f'{GHIDRA_PATH}ghidra "{os.path.abspath(filename)}"')
        return
    if temp:
    ...
    r = shouldRun()
    if r:
        os.system(f'/usr/share/ghidra/support/analyzeHeadless {out_dir} "{proj_name}" -import "{filename}"')
        os.system(f'{GHIDRA_PATH}ghidra "{proj_file}"')
